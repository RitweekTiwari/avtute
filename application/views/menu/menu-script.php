<script type="text/javascript">
  $(document).ready(function(){
    var listUrl = '<?php echo base_url("menus/list?key=all") ?>';
    ...
    $('select[name="sitemenu_parent"]').select2({
      ajax: {
    ...
    $('form#menu-add').on('submit', function(event){
      event.preventDefault();
      var thisForm = $(this);
      var $formData = $(this).serialize();
      $.ajax({
          url: '<?php echo base_url('menu/submit'); ?>',
          type: 'get',
          dataType: 'json',
          data: $formData,
          beforeSend: function(){
